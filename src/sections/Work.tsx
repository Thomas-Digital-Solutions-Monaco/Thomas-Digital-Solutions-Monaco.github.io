...
const Work = () => {
  ...
  const visible = filter === "__all" ? projects : projects.filter((p) => p.category === filter);
  const label = (c: string) => (c === "__all" ? t.work.all : c);

  return (
    <section id="work" data-snap className="section py-16 sm:py-24">
          ...
          {categories.map((c) => (
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-sm transition-colors ${
                filter === c ? "bg-brand text-white" : "border border-line text-mist hover:text-ink"
              }`}
            >
              {label(c)}
            </button>
          ...
          {visible.map((p) => (
            <article key={p.name} className="card group flex flex-col overflow-hidden">
              <div
                ...
                style={{ background: `radial-gradient(120% 120% at 30% 0%, ${p.accent}, ${p.accent}bb 60%, ${p.accent}77)` }}
              >
                <span className="text-6xl font-black text-white/90">{p.name.charAt(0)}</span>
              </div>
              <div className="flex flex-1 flex-col p-6">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {p.platforms.map((plat) => (
                    <StoreBadge key={plat} platform={plat} comingSoon={t.work.comingSoon} />
                  ))}
                </div>
              </div>
            </article>
