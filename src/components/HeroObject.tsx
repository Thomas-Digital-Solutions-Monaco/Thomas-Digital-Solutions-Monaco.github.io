...
/**
 * A detailed stylized Formula 1 car — a nod to the Monaco Grand Prix — built
 * entirely from primitives, so there are NO external model files to host and it
 * loads instantly. Features: monocoque, engine cover, airbox, long nose,
 * multi-element front & rear wings, sidepods, bargeboards, floor, halo, driver
 * helmet + visor, suspension arms, and rimmed tyres.
 */
...
/** A single suspension arm between chassis and wheel. */
const Arm = ({
  from,
  to,
}: {
  ...
  return (
    <Float speed={1.1} rotationIntensity={0.2} floatIntensity={0.55}>
      ...
      >
        ...
        <mesh position={[0.4, 0.32, 0]} rotation={[0, 0, 0]}>
          <sphereGeometry
            args={[0.163, 28, 16, 0, Math.PI * 2, Math.PI * 0.32, Math.PI * 0.28]}
          />
